...
cg::range_t cg::AnglesToRange(float start, float end, float yaw, float fov)
{
	...
	{
		range_t const ret = { 0, float(cgs->refdef.width), false };
		return ret;
	}
...
{
	float SCREEN_WIDTH = (float)cgs->refdef.width;
...
void cg::CG_FillAngleYaw(float start, float end, float yaw, float y, float h, float fov, vec4_t const color)
{
	...
	if (!range.split) {
		...
		r::R_DrawRect("white", range.x2, y, cgs->refdef.width - range.x2, h, color);
